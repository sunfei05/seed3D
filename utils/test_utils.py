...
def seedGroupMerging(pred_val, seed_softmax, seg, delta_v):
    """
    :param pred_val: N * E represent the embedding of instance
    :param seed_softmax: N * 1 represent the label of seed
    :param seg: N * 1 sem_seg label
    :param delta_v: order to segment the instance
    :return: num_instance, each point instance label
    """

    dist_mat = np.expand_dims(pred_val, axis=1) - np.expand_dims(pred_val, axis=0)
    dist_mat = np.norm(dist_mat, ord=1, axis=2)

    groupid = -1 * np.ones(pred_val.shape[0])
    numgroups = 0
    proposals = []
    point_conf_label = np.argmax(seed_softmax, axis=1)
    point_conf_val = np.max(seed_softmax, axis=1)

    # first sorted by conf_label, then sorted by conf_val
    conf_sorted_list = np.zeros(seed_softmax.shape[0]).astype(int)
    count = 0
    for i in range(2, -1, -1):
        tmp = np.copy(point_conf_val)
        not_valid_pts = (point_conf_label != i)
        # print("%d : %d" % (i, 4096 - np.sum(not_valid_pts)))
        tmp[not_valid_pts] = -1
        part_sorted_list = np.argsort(tmp)
        for j in range(part_sorted_list.shape[0] - 1, -1, -1):
            if tmp[part_sorted_list[j]] == -1:
                break
            conf_sorted_list[count] = part_sorted_list[j]
            count += 1

    # generate mask by conf_label == 2
    for ip in conf_sorted_list:
        conf_label = point_conf_label[ip]
        seg_label = seg[ip]
        if conf_label != 2:
            continue
        valid_pts = (dist_mat[ip] < delta_v) & (seg == seg_label)
        if np.sum(valid_pts) < 50:
            continue
        if groupid[ip] != -1:
            iou = float(np.sum(valid_pts & (groupid == groupid[ip]))) / np.sum(valid_pts | (groupid == groupid[ip]))
            validpt_in_gp = float(np.sum(valid_pts & (groupid == groupid[ip])))
            if iou > 0.6 or validpt_in_gp > 0.8:
                # valid_pts = valid_pts & (groupid == -1) #2类proposal质量高，直接使用，带来3个点的收益
                groupid[valid_pts] = groupid[ip]  # 如果重合超过界限，则直接将整个proposal赋值为该ins_label
                continue

        flag = 0
        for gp in range(len(proposals)):
            iou = float(np.sum(valid_pts & proposals[gp])) / np.sum(valid_pts | proposals[gp])
            validpt_in_gp = float(np.sum(valid_pts & proposals[gp])) / np.sum(valid_pts)
            if iou > 0.6 or validpt_in_gp > 0.8:  # 校验功能，但似乎该块内容并没有用到，该if循环没有进入
                ins_label = stats.mode(groupid[proposals[gp]])[0]
                groupid[valid_pts] = ins_label
                flag = 1
                break
        ratio = np.sum(valid_pts & (groupid == -1)) / np.sum(valid_pts)
        if flag == 0 and np.sum(valid_pts & (groupid == -1)) > 30 and ratio > 0.5:
            groupid[valid_pts] = numgroups
            numgroups += 1
            proposals += [valid_pts]

    # generate mask by conf_label == 1
    proposal_blank_count = -1 * np.ones(pred_val.shape[0]).astype(int)
    for ip in range(proposal_blank_count.shape[0]):
        seg_label = seg[ip]
        if point_conf_label[ip] == 1:
            valid_pts = (dist_mat[ip] < delta_v) & (seg == seg_label) & (groupid == -1)
            proposal_blank_count[i] = np.sum(valid_pts)

    label_1_sorted_list = np.argsort(proposal_blank_count).astype(int)
    for i in range(label_1_sorted_list.shape[0] - 1, -1, -1):
        ip = label_1_sorted_list[i]
        if proposal_blank_count[ip] == -1:
            break
        valid_pts = (dist_mat[ip] < delta_v) & (seg == seg_label)
        if np.sum(valid_pts) < 50 or proposal_blank_count[ip] < 30:
            continue
        if groupid[ip] != -1:
            iou = float(np.sum(valid_pts & (groupid == groupid[ip]))) / np.sum(valid_pts | (groupid == groupid[ip]))
            validpt_in_gp = float(np.sum(valid_pts & (groupid == groupid[ip]))) / np.sum(valid_pts)
            if iou > 0.6 or validpt_in_gp > 0.8:
                valid_pts = valid_pts & (groupid == -1)
                groupid[valid_pts] = groupid[ip]
                continue
        ...
        for gp in range(len(proposals)):
            ...
            if iou > 0.6 or validpt_in_gp > 0.8:
                ...
                ins_label = stats.mode(groupid[proposals[gp]])[0]
                # groupid[valid_pts] = ins_label
                groupid[ip] = ins_label
                flag = 1
                break

        ratio = np.sum(valid_pts & (groupid == -1)) / np.sum(valid_pts)
        if flag == 0 and stats.mode(groupid[valid_pts])[0] == -1 and ratio > 0.5:
            valid_pts = (valid_pts & (groupid == -1))
            groupid[valid_pts] = numgroups
            numgroups += 1
            proposals += [valid_pts]
    ...
    for ip, gid in enumerate(groupid):
        if gid == -1:
            pts_in_gp_ind = (dist_mat[ip] < delta_v)
            pts_in_gp = groupid[pts_in_gp_ind]
            pts_in_gp_valid = pts_in_gp[pts_in_gp != -1]
            if len(pts_in_gp_valid) != 0:
                groupid[ip] = stats.mode(pts_in_gp_valid)[0][0]

    return numgroups, groupid

def AdvanceGroupMerging(pts_corr, confidence, seg, label_bin):
    ...
    proposals = []
    groupseg = {}
    point_conf_label = np.argmax(confidence, axis=1)
    point_conf_val = np.max(confidence, axis=1)


    # first sorted by conf_label, then sorted by conf_val
    conf_sorted_list = np.zeros(confidence.shape[0]).astype(int)
    count = 0
    for i in range(2, -1, -1):
        tmp = np.copy(point_conf_val)
        not_valid_pts = (point_conf_label != i)
    ...
    for ip in conf_sorted_list:
        conf_label = point_conf_label[ip]
        seg_label = seg[ip]
        conf_val = point_conf_val[ip]
        if conf_label != 2:
            continue
        valid_pts = (pts_corr[ip] < label_bin[seg]) & (seg == seg_label)
        if np.sum(valid_pts) < 50:
            continue
        if groupid[ip] != -1:
            iou = float(np.sum(valid_pts & (groupid == groupid[ip]))) / np.sum(valid_pts | (groupid == groupid[ip]))
        ...
        if flag == 0 and np.sum(valid_pts & (groupid == -1)) > 30 and ratio > 0.5:
            groupid[valid_pts] = numgroups
            groupseg[numgroups] = seg_label
            numgroups += 1
            refineseg[valid_pts] = stats.mode(seg[valid_pts])[0]
            proposals += [valid_pts]

    # generate mask by conf_label == 1
    proposal_blank_count = -1 * np.ones(pts_corr.shape[0]).astype(int)
    for ip in range(proposal_blank_count.shape[0]):
        if point_conf_label[ip] == 1:
            valid_pts = (pts_corr[ip] < label_bin[seg]) & (seg == seg_label) & (groupid == -1)
            proposal_blank_count[i] = np.sum(valid_pts)
