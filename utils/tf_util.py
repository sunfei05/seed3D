...
def pairwise_distance_cosine(point_cloud):
    ...
    for idx in range(batch_size):
        idx_point_cloud = point_cloud[idx, ...]
        point_norm = tf.sqrt(tf.reduce_sum(tf.square(idx_point_cloud), axis=1))
        point_norm = tf.expand_dims(point_norm, axis=0)
        deno = tf.matmul(tf.transpose(point_norm, perm=[1, 0]), point_norm)
        nume = tf.matmul(idx_point_cloud, tf.transpose(idx_point_cloud, perm=[1, 0]))
        cosine = tf.divide(nume, deno)
        cosine = 0.5 * (1 + cosine)
        pairwise_distance.append(cosine)
