...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            loss, sem_loss, cos_loss, same_loss, neq_loss = get_loss(pred_ins, labels_pl, pred_sem,
                                                                                  sem_labels_pl, alpha=0.5)
            tf.summary.scalar('loss', loss)
            tf.summary.scalar('sem_loss', sem_loss)
            tf.summary.scalar('cos_loss', cos_loss)
            tf.summary.scalar('same_loss', same_loss)
            tf.summary.scalar('neq_loss', neq_loss)
...
def train_one_epoch(sess, ops, train_writer, epoch):
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, _, loss_val, sem_loss_val, cos_loss_val, same_loss_val, neq_loss_val = sess.run(
            [ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['sem_loss'], ops['cos_loss'],
             ops['same_loss'], ops['neq_loss']],
            feed_dict=feed_dict)
